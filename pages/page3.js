...
const SchRange = ({wmax, hmax, hori, horf}) => {
  ...
  for(let i = 0; i < num; i++){
    range.push([asfmt(asHHMM(Math.round(i*step))), i*ystep-10]);
  }
  
  return(
    <VStack>
      <Box align='center' width='1vw' height='22px' fontSize='3vh' />
...
class Main extends React.Component{
  ...
  renderSchedule(){
    const renderDayColumn = (day) => {
      return(
        <VStack>
          <Box className='text3' width={configs.wmax} height='22px'> {dayName[day]} </Box>
